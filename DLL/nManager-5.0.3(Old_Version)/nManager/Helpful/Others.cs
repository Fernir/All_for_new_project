namespace nManager.Helpful
{
    ...
    public static class Others
    {
        ...
        private static int _oldEventFireCount = -1;
        ...
        public static bool DownloadFile(string httpUrl, string fileDest)
        {
            ...
            try
            {
                lock (LockerDownload)
                {
                    _httpUrl = httpUrl;
                    _fileDest = fileDest;
                    Thread thread2 = new Thread(new ThreadStart(Others.DownloadThread)) {
                        Name = "DownloadFile"
                    };
                    thread2.Start();
                    Thread.Sleep(200);
        ...
        private static void DownloadThread()
        {
            try
            {
                ...
                try
                {
                    new WebClient().DownloadFile(_httpUrl, _fileDest);
                }
                catch (Exception exception)
                {
