    ...
    public static class Party
    {
        ...
        public static uint GetPartyNumberPlayers()
        {
            try
            {
                ...
                if (partyPointer > 0)
                {
                    uint num2 = Memory.WowMemory.Memory.ReadUInt(partyPointer + 0xcc);
                    if ((num2 > 0) && (num2 <= 40))
                    {
                        return num2;
                    }
        ...
        public static uint GetPartyPointer(PartyEnums.PartyType partyType = 1)
        {
            try
            {
                ...
                return Memory.WowMemory.Memory.ReadUInt(Memory.WowProcess.WowModule + ((uint) (0xe031f4 + ((partyType - 1) * ((PartyEnums.PartyType) 4)))));
            }
            catch (Exception exception)
            {
