    ...
    public static class Lua
    {
        public static string GetLocalizedText(string commandline)
        {
            try
            {
                ...
                string[] asm = new string[] { "call " + (Memory.WowProcess.WowModule + 0x3c47), "test eax, eax", "je @out", "mov ecx, eax", "push -1", "mov edx, " + dwAddress, "push edx", "call " + (Memory.WowProcess.WowModule + 0x2fe479), "mov [" + num2 + "], eax", "test eax, eax", "je @out", "push eax", "call " + (Memory.WowProcess.WowModule + 0x6c9e50), "add esp, 4", "@out:", "retn" };
            ...
            try
            {
                ...
                if (command.Replace(" ", "").Length > 0)
                {
                    ...
                    if (dwAddress > 0)
                    {
                        ...
                        string[] collection = new string[] { "mov eax, " + dwAddress, "push 0", "push eax", "push eax", "mov eax, " + (Memory.WowProcess.WowModule + 0x27dd1), "call eax", "add esp, 0xC", "@out:", "retn" };
                        if (!notInGameMode)
                        {
                            List<string> list = new List<string> {
                                "call " + (Memory.WowProcess.WowModule + ((uint) 0x3c47)),
                                "test eax, eax",
                                "je @out"
                            };
