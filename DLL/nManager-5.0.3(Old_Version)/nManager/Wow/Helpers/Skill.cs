namespace nManager.Wow.Helpers
{
    ...
    public class Skill
    {
        private static uint GetBaseSkill(SkillLine skill)
        {
            try
            {
                uint num2 = Memory.WowMemory.Memory.ReadUInt(nManager.Wow.ObjectManager.ObjectManager.Me.GetBaseAddress + Descriptors.StartDescriptors) + 0x1bf0;
                uint num3 = 0;
                uint num4 = 0;
                do
                {
                    if (((SkillLine) Memory.WowMemory.Memory.ReadShort(num2 + num3)) == skill)
                    {
                        return num4;
                    }
                    num4++;
        ...
        public static int GetMaxValue(SkillLine skill)
        {
            try
            {
                uint baseSkill = GetBaseSkill(skill);
                if (baseSkill > 0)
                {
                    uint num3 = Memory.WowMemory.Memory.ReadUInt(nManager.Wow.ObjectManager.ObjectManager.Me.GetBaseAddress + Descriptors.StartDescriptors) + 0x1bf0;
                    return Memory.WowMemory.Memory.ReadShort(((baseSkill * 2) + num3) + 0x400);
                }
                return 0;
            }
            catch (Exception exception)
        ...
        public static int GetValue(SkillLine skill)
        {
            try
            {
                ...
                {
                    uint num3 = Memory.WowMemory.Memory.ReadUInt(nManager.Wow.ObjectManager.ObjectManager.Me.GetBaseAddress + Descriptors.StartDescriptors) + 0x1bf0;
                    return Memory.WowMemory.Memory.ReadShort(((baseSkill * 2) + num3) + 0x200);
                }
                return 0;
            }
            catch (Exception exception)
