namespace nManager
{
    ...
    {
        private static Language _toitao = new Language();

        public static string Get(Id id)
        {
            if ((_toitao != null) && (_toitao.Translations.Count != 0))
            {
                if (_toitao.Translations.Count == 0)
                {
                    Load("English.xml");
                }
                foreach (Translation translation in from v in _toitao.Translations
                    where v.Id == id
                    select v)
                {
                ...
                foreach (Translation translation2 in XmlSerializer.Deserialize<Language>(Application.StartupPath + @"\Data\Lang\English.xml").Translations)
                {
                    ...
                    {
                        _toitao.Translations.Add(translation2);
        ...
        public static bool Load(string fileName = "English.xml")
        {
            ...
            _toitao = new Language();
            _toitao = XmlSerializer.Deserialize<Language>(Application.StartupPath + @"\Data\Lang\" + fileName) ?? XmlSerializer.Deserialize<Language>(Application.StartupPath + @"\Data\Lang\English.xml");
            return true;
        }
        ...
        {
            private List<Translate.Translation> _huxaufagaodeunXaun = new List<Translate.Translation>();

            public List<Translate.Translation> Translations
            {
                get
                {
                    return this._huxaufagaodeunXaun;
                }
                set
                {
                    this._huxaufagaodeunXaun = value;
                }
            }
        }
        ...
        {
            private string _guwueniweTuapip = "";
            private nManager.Translate.Id _iqearaweoxear;
            ...
            public string Text
            {
                get
                {
                    return this._guwueniweTuapip;
                }
                set
                {
                    this._guwueniweTuapip = value;
                }
            }
        }
    }
