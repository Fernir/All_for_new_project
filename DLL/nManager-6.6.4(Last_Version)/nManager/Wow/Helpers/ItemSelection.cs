namespace nManager.Wow.Helpers
{
    ...
    public class ItemSelection
    {
        public static bool CheckItemStats(string link)
        {
            ...
            {
                string otoqib = strArray[index];
                WoWStatistic item = Kaumie(otoqib);
                if ((item != WoWStatistic.None) && !EquipmentAndStats._ewilosiegulWoacuomeTione.Contains(item))
                {
                    flag = false;
                }
                index += 2;
        ...
        private static WoWStatistic Kaumie(string otoqib)
        {
            switch (otoqib)
            {
