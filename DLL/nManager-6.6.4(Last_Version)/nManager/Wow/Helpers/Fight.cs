    ...
    public class Fight
    {
        public static UInt128 StartFight(UInt128 guid = new UInt128())
        {
            ...
            try
            {
                ...
                if (!unit.Attackable)
                {
                    nManagerSetting.AddBlackList(unit.Guid, 0x4e20);
                }
            ...
            Label_00C7:
                ...
                if ((nManager.Wow.ObjectManager.ObjectManager.Me.InCombat && !CombatClass.InRange(unit)) || TraceLine.TraceLineGo(unit.Position))
                {
                    ...
                    while (((!nManager.Wow.ObjectManager.ObjectManager.Me.IsDeadMe && !unit.IsDead) && (unit.Attackable && !unit.IsLootable)) && ((((unit.Health > 0) && unit.IsValid) && (MovementManager.InMovement && InFight)) && (Usefuls.InGame && (TraceLine.TraceLineGo(unit.Position) || !CombatClass.InAggroRange(unit)))))
                    {
                ...
                if ((unit.IsDead || !unit.IsValid) || unit.Attackable)
                {
                ...
                while (((!nManager.Wow.ObjectManager.ObjectManager.Me.IsDeadMe && !unit.IsDead) && (unit.Attackable && unit.IsValid)) && ((InFight && unit.IsValid) && !nManager.Wow.ObjectManager.ObjectManager.Me.InTransport))
                {
        ...
        public static UInt128 StartFightDamageDealer(UInt128 guid = new UInt128())
        {
            ...
            Label_00C9:
                ...
                while ((!nManager.Wow.ObjectManager.ObjectManager.Me.IsDeadMe && !unit.IsDead) && ((unit.IsValid && InFight) && unit.Attackable))
                {
                    if (!unit.Position.IsValid)
                    {
                        return unit.Guid;
