namespace nManager.Wow.Helpers
{
    ...
    {
        private static bool _ajeoleuneute;
        private static List<Spell> _riuteudiufei = new List<Spell>();
        public static Dictionary<uint, SpellInfoLua> _spellInfos = new Dictionary<uint, SpellInfoLua>();
        private static List<uint> _unequnauhiajaLaege = new List<uint>();
        ...
        public static List<uint> AllInteractMountId()
        {
            ...
            {
                Logging.WriteError("AllInteractMountId(): " + exception, true);
            }
        ...
        public static List<uint> DruidMountId()
        {
            ...
            {
                Logging.WriteError("DruidMountId(): " + exception, true);
            }
            return new List<uint>();
        }
                ...
                lock (SpellManagerLocker)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(str2))
                    {
                        ...
                        if (strArray2.Length == 6)
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(strArray2[2]) && int.TryParse(strArray2[2].Replace(".", ","), out num))
                            {
                                lua.CastTime = num;
                            }
                            if (!string.IsNullOrWhiteSpace(strArray2[3]) && float.TryParse(strArray2[3].Replace(".", ","), out num2))
                            {
                                lua.MinRange = num2;
                            }
        ...
        public static bool HasSpell(int spellId)
        {
            ...
            return ((Memory.WowMemory.Memory.ReadInt(Memory.WowMemory.Memory.ReadUInt((Memory.WowProcess.WowModule + 0xf9c5d8) + ((uint) ((spellId >> 5) * 4)))) & ~(((int) 1) << spellId)) != 0);
        }
        ...
        public static List<uint> HerbsInteractMountId()
        {
            ...
            {
                Logging.WriteError("HerbsInteractMountId(): " + exception, true);
            }
            return new List<uint>();
        }
        ...
        public static bool IsSpellOnCooldown(uint spellId, uint categoryId = 0, uint startRecoveryCategoryId = 0)
        {
            ...
            {
                SpellCooldownEntry entry2 = entry;
                if ((entry2.GCDDuration <= 0) && ((entry2.SpellId == spellId) || ((entry2.SpellCategoryId == categoryId) && (categoryId != 0))))
                {
                    if (((entry2.SpellId == spellId) && (entry2.CategoryCooldownStartTime > 0)) && (entry2.CategoryCooldownStartTime != entry2.StartTime))
                    {
                        entry2.StartTime = entry2.CategoryCooldownStartTime;
                    }
                    int getWoWTime = Usefuls.GetWoWTime;
                    int num2 = getWoWTime - entry2.StartTime;
                    if (((entry2.SpellOrItemCooldownDuration > 0) && (entry2.SpellId == spellId)) && (entry2.SpellOrItemCooldownDuration >= num2))
                    {
                        return true;
                    }
                    if (entry2.CategoryCooldownDuration > 0)
                    {
                        num2 = getWoWTime - entry2.CategoryCooldownStartTime;
                        if (entry2.CategoryCooldownDuration >= num2)
                        {
                            return true;
                        }
                    }
            ...
            try
            {
                lock (SpellManagerLocker)
                {
                    if (_riuteudiufei.Count <= 0)
                    {
                        ...
                        Logging.Write("Character's SpellBook fully loaded. Found " + _unequnauhiajaLaege.Count + " spells, mounts and professions.");
                        Logging.Status = "Character SpellBook loaded.";
                        _riuteudiufei = list;
                    }
                }
                return _riuteudiufei;
            }
            catch (Exception exception)
            {
        ...
        public static List<uint> SpellBookID()
        {
            lock (SpellManagerLocker)
            {
                try
                {
                    ...
                    if (_unequnauhiajaLaege.Count <= 0)
                    {
                        _ajeoleuneute = true;
                        List<uint> list = new List<uint>();
                        uint num = Memory.WowMemory.Memory.ReadUInt(Memory.WowProcess.WowModule + 0xf9c5dc);
                        uint num2 = Memory.WowMemory.Memory.ReadUInt(Memory.WowProcess.WowModule + 0xf9c5e0);
                        for (uint i = 0; i < num; i++)
                        {
                            uint dwAddress = Memory.WowMemory.Memory.ReadUInt(num2 + (i * 4));
                            UdigoediDiemecaik diemecaik = (UdigoediDiemecaik) Memory.WowMemory.Memory.ReadObject(dwAddress, typeof(UdigoediDiemecaik));
                            if (((diemecaik.TabId <= 1) || (diemecaik.TabId > 4)) && (diemecaik.State == UdigoediDiemecaik.SpellState.Known))
                            {
                                list.Add(diemecaik.ID);
                            }
                            Application.DoEvents();
                        }
                        uint num5 = Memory.WowMemory.Memory.ReadUInt(Memory.WowProcess.WowModule + 0xf9c638);
                        uint num6 = Memory.WowMemory.Memory.ReadUInt(Memory.WowProcess.WowModule + 0xf9c63c);
                        ...
                        uint num9 = Memory.WowMemory.Memory.ReadUInt(Memory.WowProcess.WowModule + 0xf9c6d4);
                        uint num10 = Memory.WowMemory.Memory.ReadUInt(Memory.WowProcess.WowModule + 0xf9c6cc);
                        ...
                        _unequnauhiajaLaege = list;
                        _ajeoleuneute = false;
                        SpellBookLoaded = true;
                    }
                    return _unequnauhiajaLaege;
                }
                catch (Exception exception)
                {
        ...
        {
            string randomString = Others.GetRandomString(Others.Random(4, 10));
            return (Others.ToInt32(Lua.LuaDoString(string.Concat(new object[] { "_, ", randomString, " = GetSpellCharges(", spellId, ")" }), randomString, false)) > 0);
        }
            ...
            lock (SpellManagerLocker)
            {
                try
                {
                    ...
                    for (uint i = 0; i < num; i++)
                    {
                        ...
                        if (((diemecaik.TabId <= 1) || (diemecaik.TabId > 4)) && (diemecaik.State == UdigoediDiemecaik.SpellState.Known))
                        {
                            if (!_unequnauhiajaLaege.Contains(diemecaik.ID))
                            {
                                _unequnauhiajaLaege.Add(diemecaik.ID);
                            }
                            Spell item = SpellInfoLUA(diemecaik.ID);
                            if (!_riuteudiufei.Contains(item))
                            {
                                _riuteudiufei.Add(item);
                            }
                        }
                        Application.DoEvents();
                    }
                    ...
                    for (uint j = 0; j < num5; j++)
                    {
                        ...
                        if (num8 > 0)
                        {
                            if (!_unequnauhiajaLaege.Contains(num8))
                            {
                                _unequnauhiajaLaege.Add(num8);
                            }
                            Spell spell2 = SpellInfoLUA(num8);
                            if (!_riuteudiufei.Contains(spell2))
                            {
                                _riuteudiufei.Add(spell2);
                            }
                        }
                        Application.DoEvents();
                    }
                    ...
                    Memory.WowMemory.GameFrameLock();
                    foreach (Spell spell3 in _riuteudiufei)
                    {
                        spell3.Sougie();
                    }
                    Memory.WowMemory.GameFrameUnLock();
                    ...
                    Logging.Write("Character's SpellBook fully updated. Found " + _unequnauhiajaLaege.Count + " spells, mounts and professions.");
                }
                catch (Exception exception)
                {
        ...
        public static List<SpellCooldownEntry> GetAllSpellsOnCooldown
        {
            get
            {
                ...
                uint dwAddress = memory.ReadUInt((Memory.WowProcess.WowModule + 0xdf75a0) + 8);
        ...
        public class SpellListManager
        {
            ...
            internal static void OliafVaceDoewueqe(string toeraKetaheo)
            {
                try
                {
                    lock (LoadSpellListLock)
                    {
                        if (ListSpell == null)
                        {
                            ...
                            foreach (string str in Others.ReadFileAllLines(toeraKetaheo))
                            {
        ...
        private struct UdigoediDiemecaik
        {
            [FieldOffset(4)]
