namespace nManager.Wow.Helpers
{
    ...
    {
        private static List<uint> _ikoavuot;
        private static List<uint> _onudid;
        ...
        public static void EquipFishingPoles(string fishingPoleName = "")
        {
            try
            {
                if (ReCheckFishingPoleTimer.IsReady && !IsEquipedFishingPoles(""))
                {
                    ...
                    {
                        foreach (int num in EbijeotagaejeuPa())
                        {
        ...
        public static bool IsEquipedFishingPoles(string fishingPoleName = "")
        {
            try
            {
                ...
                if (EbijeotagaejeuPa().Contains((uint) equippedItem.Entry))
                {
                    return true;
                }
            }
        ...
        public static uint SearchBobber()
        {
            try
            {
                List<uint> displayId = new List<uint> { 0x29c, 0x7414, 0x2f81, 0x479c };
                foreach (WoWGameObject obj2 in nManager.Wow.ObjectManager.ObjectManager.GetWoWGameObjectByDisplayId(displayId))
                {
        ...
        private static IEnumerable<uint> _eqioxedusoep
        {
            get
            {
                try
        ...
        private static List<uint> _meunoamuimEw
        {
            get
            {
                try
