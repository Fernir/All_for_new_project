    ...
    public class ItemsManager
    {
        ...
        public static List<WoWItem> GetAllFoodItems()
        {
            ...
            try
            {
                ...
                {
                    if (((item.GetItemInfo.ItemType == "Consumable") && (item.GetItemInfo.ItemSubType == "Food & Drink")) && (!list.Contains(item) && !item.Name.Contains("\"")))
                    {
                        list.Add(item);
                    }
                }
        ...
        public static List<string> GetAllQuestsItems()
        {
            ...
            try
            {
                ...
                foreach (WoWItem item in objectWoWItem)
                {
                    if (((item.GetItemInfo.ItemType == "Quest") && !list.Contains(item.Name)) && ((item.GetItemInfo.ItemRarity == 1L) && !item.Name.Contains("\"")))
                    {
                        list.Add(item.Name);
                    }
                }
        ...
        public static List<string> GetAllReagentsItems()
        {
            ...
            try
            {
                ...
                foreach (WoWItem item in objectWoWItem)
                {
                    if (((item.GetItemInfo.ItemType == "Tradeskill") && !list.Contains(item.Name)) && !item.Name.Contains("\""))
                    {
                        list.Add(item.Name);
                    }
                }
        ...
        public static int GetItemIdByName(string name)
        {
            if (name != "")
            {
                try
                {
                    ...
                    {
                        string randomString = Others.GetRandomString(Others.Random(4, 10));
                        Lua.LuaDoString("local nameItem = \"" + name + "\" _,itemLink,_,_,_,_,_,_,_,_,_  = GetItemInfo(nameItem); if itemLink == nil then " + randomString + " = 0 else _,_," + randomString + " = string.find(itemLink, 'item:(%d+)') end", false, true);
        ...
        {
            string randomString = Others.GetRandomString(Others.Random(4, 10));
            Lua.LuaDoString(string.Concat(new object[] { randomString, " = tostring(PlayerHasToy(", entry, "));" }), false, true);
        ...
        private static List<WoWItem> Neasofeav(IEnumerable<WoWItem> fuarujaecoatuaGih, nManager.Wow.Enums.WoWItemClass gidaf)
        {
            try
            {
                List<WoWItem> list = new List<WoWItem>();
                foreach (WoWItem item in fuarujaecoatuaGih)
                {
                    if (item.GetItemInfo.ItemType == gidaf.ToString())
                    {
                        list.Add(item);
                    }
                }
        ...
        private static List<WoWItem> SeivaepuxoOn(IEnumerable<WoWItem> fuarujaecoatuaGih, WoWItemTradeGoodsClass vikoka)
        {
            try
            {
                List<WoWItem> list = new List<WoWItem>();
                foreach (WoWItem item in fuarujaecoatuaGih)
                {
                    if (item.GetItemInfo.ItemSubType == vikoka.ToString())
                    {
                        list.Add(item);
                    }
                }
        ...
        {
            Lua.LuaDoString("UseToy(" + entry + ");", false, true);
        }

        private static WoWItem Weumeor(IEnumerable<WoWItem> fuarujaecoatuaGih)
        {
            try
            {
                WoWItem item = new WoWItem(0);
                int itemLevel = 0;
                foreach (WoWItem item2 in fuarujaecoatuaGih)
                {
