    ...
    public class CGUnit_C__GetFacing
    {
        public static float GetFacing(uint baseAddress)
        {
            try
            {
                if (baseAddress > 0)
                {
                    uint num = Memory.WowMemory.Memory.ReadUInt(Memory.WowMemory.Memory.ReadUInt(baseAddress) + 0xe4);
                    if (num <= 0)
                    {
                        return 0f;
                    }
                    ...
                    string[] asm = new string[] { "call " + (Memory.WowProcess.WowModule + 0x8dd5a), "test eax, eax", "je @out", "mov ecx, " + baseAddress, "call " + num, "mov eax, " + dwAddress, "fstp dword [eax]", "@out:", "retn" };
