        ...
        public override void Run()
        {
            if (!this.IgnoreSmeltingZone)
            {
                ...
                if (npcNearby.Position.DistanceTo(nManager.Wow.ObjectManager.ObjectManager.Me.Position) > 10f)
                {
                    ...
                    Thread.Sleep(700);
                    while ((MovementManager.InMovement && nManager.Products.Products.IsStarted) && ((Usefuls.InGame && !nManager.Wow.ObjectManager.ObjectManager.Me.InInevitableCombat) && !nManager.Wow.ObjectManager.ObjectManager.Me.IsDeadMe))
                    {
                        if (timer.IsReady)
                        {
