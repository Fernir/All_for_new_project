namespace nManager.Wow.MemoryClass.Magic
{
    ...
    public static class SMemory
    {
        ...
        private static int Voejacaim(IntPtr arugeadado, uint taunaefid, IntPtr ubejutuiroreIcUbofaig, int ivaudi)
        {
            IntPtr zero = IntPtr.Zero;
            if (!Imports.WriteProcessMemory(arugeadado, taunaefid, ubejutuiroreIcUbofaig, ivaudi, out zero))
            {
                return 0;
            }
            return (int) zero;
        ...
        public static bool WriteASCIIString(IntPtr hProcess, uint dwAddress, string Value)
        {
            ...
            try
            {
                ...
                num = Voejacaim(hProcess, dwAddress, zero, ivaudi);
                if (ivaudi != num)
                {
                    throw new Exception();
                }
        ...
        public static bool WriteBytes(IntPtr hProcess, uint dwAddress, byte[] lpBytes, int nSize)
        {
            ...
            try
            {
                ...
                num = Voejacaim(hProcess, dwAddress, zero, nSize);
                if (nSize != num)
                {
        ...
        public static bool WriteObject(IntPtr hProcess, uint dwAddress, object objBuffer, Type objType)
        {
            ...
            try
            {
                ...
                num2 = Voejacaim(hProcess, dwAddress, zero, cb);
                if (cb != num2)
                {
