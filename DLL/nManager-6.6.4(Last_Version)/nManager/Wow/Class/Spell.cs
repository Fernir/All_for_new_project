namespace nManager.Wow.Class
{
    ...
    public class Spell
    {
        ...
        public Spell(string spellName)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (spell == null)
                    {
                        spell = new Spell(SpellManager.SpellListManager.SpellIdByName(spellName).FirstOrDefault<uint>());
                        if (spell.Id == 0)
                        {
                            spell = null;
                        }
                ...
                else
                {
                    string str = "";
                    string str2 = "";
                    if (spell.Name != spell.NameInGame)
                    {
                        str = "NameClient: " + spell.NameInGame + ", ";
                    }
                    if (spell.HaveBuff)
                    {
                        str2 = ", HaveBuffOrPassive: " + spell.HaveBuff;
                    }
                    Logging.WriteDebug(string.Concat(new object[] { "Spell(string spellName): Id: ", spell.Id, ", Name: ", spell.Name, ", ", str, "Known: ", spell.KnownSpell, str2 }));
                    this.Id = spell.Id;
                    ...
                    this.SpellHasCharges = spell.SpellHasCharges;
            ...
            lock (this)
            {
                try
                {
                    ...
                    if ((spellInfo.ID > 0) && (spellId == spellInfo.ID))
                    {
                        ...
                        this.SpellHasCharges = SpellManager.SpellHasCharges(this.Id);
                        this.Ids.Add(this.Id);
        ...
        {
            SpellManager.CastSpellByIDAndPosition(this.Id, position);
        }
        ...
        public void Launch(bool StopMove, bool waitIsCast = true, bool ignoreIfCast = false, string unitId = null)
        {
            try
            {
                if ((!nManager.Wow.ObjectManager.ObjectManager.Target.IsValid || ((unitId != null) && (unitId != "target"))) || !TraceLine.TraceLineGo(nManager.Wow.ObjectManager.ObjectManager.Target.Position, nManager.Wow.ObjectManager.ObjectManager.Me.Position, CGWorldFrameHitFlags.HitTestBoundingModels | CGWorldFrameHitFlags.HitTestMovableObjects | CGWorldFrameHitFlags.HitTestWMO))
                {
                    Memory.WowMemory.GameFrameUnLock();
                    if (StopMove)
                    {
        ...
        public void LaunchOnSelf()
        {
            try
            {
                this.Launch(!(this.CastTime == 0f), true, false, "player");
            }
            catch (Exception exception)
            {
        ...
        public void LaunchOnSelf(bool StopMove, bool waitIsCast = true, bool ignoreIfCast = false)
        {
            try
            {
                this.Launch(StopMove, waitIsCast, ignoreIfCast, "player");
            }
            catch (Exception exception)
            {
        ...
        public bool IsSpellUsable
        {
            get
            {
                try
                {
                    return (this.KnownSpell && this.IsSpellUsableIgnoreUnknown);
                }
                catch (Exception exception)
                {
        ...
        public bool IsSpellUsableIgnoreUnknown
        {
            get
            {
                try
                {
                    return ((!this.SpellHasCharges || (this.GetSpellCharges > 0)) && SpellManager.IsSpellUsableLUA(this));
                }
                catch (Exception exception)
                {
                    Logging.WriteError("Spell > IsSpellUsableIgnoreUnknown: " + exception, true);
                }
                return false;
            }
        }
