namespace nManager.Helpful
{
    ...
    {
        private static List<int> _anoipuBainal;
        ...
        public static List<BuildingID> GetBuildingOwnedList()
        {
            try
            {
                ...
                Lua.LuaDoString("mygb = \"\"; local buildings = C_Garrison.GetBuildings(LE_GARRISON_TYPE_6_0)  for i = 1, #buildings do   mygb =  mygb.. tostring(buildings[i].buildingID) .. \"|\" end ", false, true);
        ...
        {
            string randomString = Others.GetRandomString(Others.Random(4, 10));
            Lua.LuaDoString(randomString + " = C_Garrison.GetGarrisonInfo(LE_GARRISON_TYPE_6_0)", false, true);
            return Others.ToInt32(Lua.GetLocalizedText(randomString));
        }
        ...
        public static List<int> GarrisonMapIdList
        {
            get
            {
                try
                {
                    if (_anoipuBainal == null)
                    {
                        _anoipuBainal = new List<int>();
                        foreach (string str in Others.ReadFileAllLines(Application.StartupPath + @"\Data\garrisonMapIdList.txt"))
                        {
                            if (!string.IsNullOrWhiteSpace(str))
                            {
                                _anoipuBainal.Add(Others.ToInt32(str));
                            }
                        }
                    }
                    return _anoipuBainal;
                }
                catch (Exception exception)
                {
